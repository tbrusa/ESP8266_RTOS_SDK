...
/////////////////////////////////////Fake Functions////////////////////////////////////////////
// These are used cause "atomic_fetch_add_4" is somehow missing for xtensa-lx106-elf-gcc
//////////////////////////////////////////////////////////////////////////////////////////////

static void atomic_fetch_add_fake(int *ref, int val)
{
	*ref += val;
}

static void atomic_fetch_sub_fake(int *ref, int val)
{
	*ref -= val;
}

static int atomic_compare_exchange_strong_fake(int *ref, int *comp, int new)
{
	if (*ref == *comp)
	{
		*ref = new;
...
esp_err_t esp_eth_driver_uninstall(esp_eth_handle_t hdl)
{
    ...
    if (!atomic_compare_exchange_strong_fake(&eth_driver->fsm, &expected_fsm, ESP_ETH_FSM_STOP)) {
    ...
    if (!atomic_compare_exchange_strong_fake(&eth_driver->ref_count, &expected_ref_count, 0)) {
...
esp_err_t esp_eth_start(esp_eth_handle_t hdl)
{
    ...
    // check if driver has started
    esp_eth_fsm_t expected_fsm = ESP_ETH_FSM_STOP;
    if (!atomic_compare_exchange_strong_fake(&eth_driver->fsm, &expected_fsm, ESP_ETH_FSM_START)) {
...
esp_err_t esp_eth_increase_reference(esp_eth_handle_t hdl)
{
    ...
    esp_eth_driver_t *eth_driver = (esp_eth_driver_t *)hdl;
    ETH_CHECK(eth_driver, "ethernet driver handle can't be null", err, ESP_ERR_INVALID_ARG);
    atomic_fetch_add_fake(&eth_driver->ref_count, 1);
    return ESP_OK;
err:
    return ret;
}
...
esp_err_t esp_eth_decrease_reference(esp_eth_handle_t hdl)
{
    ...
    esp_eth_driver_t *eth_driver = (esp_eth_driver_t *)hdl;
    ETH_CHECK(eth_driver, "ethernet driver handle can't be null", err, ESP_ERR_INVALID_ARG);
    atomic_fetch_sub_fake(&eth_driver->ref_count, 1);
    return ESP_OK;
err:
    return ret;
}
